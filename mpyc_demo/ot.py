...
async def main():
    ...
    if mpc.pid == 0:
        ...
        # pid在[t+1, 2t]之间的用户, 随机选择接收pid-t的用户发送的两条消息的哪一条
        choice[mpc.pid - t - 1] = random.randint(0, 1)
        print(
            f'You are receiver {mpc.pid - t} with random choice bit {choice[mpc.pid - t - 1]}.')

    # mpc相当于省略了mpc.start()和mpc.shutdown()
    async with mpc:
        secint = mpc.SecInt()
        for i in range(1, t+1):
            x = mpc.input([secint(message[i - 1][0]),
                          secint(message[i - 1][1])], i)
            # 是否接收pid为t+1的用户发送的mpc.input
